...
func TestReElection(t *testing.T) {
	...
	fmt.Println("Passed initially")

	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	fmt.Println("Test leader disconnected")
	cfg.checkOneLeader()
	fmt.Println("Passed after disconnect")

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	fmt.Println("Test old leader rejoins")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	fmt.Println("Passed after old leader rejoins")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	fmt.Println("Passed after no quorum test")

	// if a quorum arises, it should elect a leader.
	fmt.Println("Test for quorum arises")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	fmt.Println("Passed when quorum arises")
